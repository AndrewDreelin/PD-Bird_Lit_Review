...
rev<-read.csv("PD_Bird_LR_Jan_22.csv") #read in final data set
...
#^^making sure "general" studies that include species from other categories count appropriately-- yes!

rev_study<-rev %>% distinct(Ref_Num, .keep_all = TRUE) #filtering analyses down to unique studies

mopl_study<-rev_study[grep("MOPL",rev_study$Bird_Species),] #filtering by category again
...
BUOW_count<-lengths(regmatches(rev_study$Bird_Species, gregexpr("BUOW", rev_study$Bird_Species)))
BUOW_count<-as.table(BUOW_count)
table(BUOW_count)

#number of studies involving MOPL
MOPL_count<-lengths(regmatches(rev_study$Bird_Species, gregexpr("MOPL", rev_study$Bird_Species)))
MOPL_count<-as.table(MOPL_count)
table(MOPL_count)

#number of studies involving FEHA
FEHA_count<-lengths(regmatches(rev_study$Bird_Species, gregexpr("FEHA", rev_study$Bird_Species)))
FEHA_count<-as.table(FEHA_count)
table(FEHA_count)

#number of studies for all species
rev_study$Bird_Species<-as.character(rev_study$Bird_Species)

pubs_per_species<-table(unlist(strsplit(rev_study$Bird_Species, ', ')))

view(pubs_per_species) #works!
...
hyp1$Abundance_Change = factor(hyp1$Abundance_Change, levels = c('Decrease','Neutral','Increase'))
...
ggplot(hyp1) + #species in manual order of net support
  ...
  scale_fill_manual("Abundance Change", values = cbPalette) + 
...
bm<-ggplot(NAm_map, aes(x = long, y = lat, group = group), fill = "#c9c9c9") + 
  geom_polygon() + 
  geom_polygon(data = plains, color = "#c9c9c9") +
