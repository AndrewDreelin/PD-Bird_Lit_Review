...
#Figure 1
ggplot(rev1) +
  geom_bar(aes(y=factor(Bird_Category,levels=c("General","Raptor","Burrowing Owl","Mountain Plover")), fill = Hypothesis_1), width = 0.5) +
...
bm0<-ggplot(data = NAm_map, aes(x = long, y = lat, group = group), fill = "#c9c9c9") + 
  geom_polygon() + 
  geom_polygon(data = plains, color = "#c9c9c9", size = 0.1) +
...
bm<-ggplot()+
  geom_polygon(data = NAm_map, aes(x = long, y = lat, group = group), color = "#c9c9c9") + 
  geom_polygon(data = plains, aes(x = long, y = lat, group = group), color = "#c9c9c9", size = 0.1, inherit.aes = FALSE) +
...
p_dummy<-bm0 + #for legend
  ...
  scale_y_latitude(breaks = c(50, 40, 30)) +
  scale_color_manual(values = c("Breeding Season" = "#c25c4d","Breeding, Postbreeding Migration" = "#dda821","Nonbreeding Season" = "#337197", "Year-round" = "#775178", "Breeding, Nonbreeding" = "#7cad58", "Not applicable" = "gray")) +
  guides(color = guide_legend(override.aes = list(size = 10, shape = c(15,15,15,15,15)))) + 
  theme(legend.position="bottom",
        legend.text=element_text(size=11),
        legend.title=element_text(size=12),
        axis.title=element_text(size=18),
